...
class OpRegGenImpl(transformer.CodeGenerator):
  ...
  def visit_FunctionDef(self, node):
    ...
    if len(expected_args) != len(all_func_args):
      raise KeyError(
          f'Composition arguments for {op_name} do not match the registration. {expected_args} vs {all_func_args}'
      )

    cxx_reg_code = [f'\nREGISTER_OP("{op_name}")']
    cxx_reg_code.extend(f'.Input("{input_}")' for input_ in inputs)
    for attr in attrs:
      py_str = attr.replace('"', "'")
      cxx_reg_code.append(f'.Attr("{py_str}")')
    for attr in all_dec_args.get('derived_attrs', []):
      py_str = attr.replace('"', "'")
      cxx_reg_code.append(f'.Attr("{py_str}")')
    cxx_reg_code.extend(
        f'.Output("{output_}")' for output_ in all_dec_args.get('outputs', []))
    cxx_reg_code[-1] += ';\n'
    self.emit('\n    '.join(cxx_reg_code))
